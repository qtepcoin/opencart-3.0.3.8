			...
			foreach ($query->rows as $language) {
				...
				if ($query->num_rows) {
					foreach ($query->rows as $row) {
						$this->db->query("UPDATE " . DB_PREFIX . "product_description SET tag = '" . $this->db->escape(strtolower($row['tags'])) . "' WHERE extension_id = '" . (int)$row['extension_id'] . "' AND language_id = '" . (int)$language['language_id'] . "'");
